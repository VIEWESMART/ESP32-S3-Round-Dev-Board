...
#define LCD_H_RES (410) // 宽
#define LCD_V_RES (502) // 高
...
#define ENCODER_GPIO_A (GPIO_NUM_6)
...
static const st7801n_lcd_init_cmd_t lcd_init_cmds[] = {
    {0x11, (uint8_t[]){0x00}, 0, 100},//退出睡眠模式
    {0x13, (uint8_t[]){0x00}, 0, 10},//开启正常显示模式
    {0x20, (uint8_t[]){0x00}, 0, 10},//关反色
    {0x35, (uint8_t[]){0x00}, 1, 10},//开启抗撕裂
    {0x3A, (uint8_t[]){0x55}, 1, 10},//像素格式565
    {0x53, (uint8_t[]){0x20}, 1, 10}, // 亮度控制开关
    {0x51, (uint8_t[]){0xff}, 1, 80}, // 亮度0-ff
    {0x2A, (uint8_t[]){0x00, 0x00, 0x01, 0x99}, 4, 0}, // 列设置，从0-409
    {0x2B, (uint8_t[]){0x00, 0x00, 0x01, 0xf5}, 4, 0}, // 行设置，从0-501
    {0x29, (uint8_t[]){0x00}, 0, 0},  //开启显示
};
...
static void lvgl_port_task(void *arg)
{
    ...
    const spi_bus_config_t buscfg = ST7801N_PANEL_BUS_QSPI_CONFIG(PIN_NUM_LCD_PCLK,
                                                                 PIN_NUM_LCD_DATA0,
                                                                 PIN_NUM_LCD_DATA1,
                                                                 PIN_NUM_LCD_DATA2,
                                                                 PIN_NUM_LCD_DATA3,
    ...
    const esp_lcd_panel_io_spi_config_t io_config = ST7801N_PANEL_IO_QSPI_CONFIG(PIN_NUM_LCD_CS, notify_lvgl_flush_ready, &disp_drv);
    // 将 LCD 连接到 SPI 总线
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config, &io_handle));

    ESP_LOGI(TAG, "安装 ST7801N 的 LCD 驱动程序");
    esp_lcd_panel_handle_t panel_handle = NULL;
    const st7801n_vendor_config_t vendor_config = {
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_st7801n(io_handle, &panel_config, &panel_handle));
